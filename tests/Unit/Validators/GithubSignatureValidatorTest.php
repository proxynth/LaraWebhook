...
describe('GithubSignatureValidator validate', function () {
    ...
    it('validates correct signature', function () {
        ...
        $signature = incomingSignature('sha256='.hash_hmac('sha256', $payload, $this->secret));
    ...
    it('throws exception for missing sha256 prefix', function () {
        $payload = '{"action": "opened"}';
        $signature = incomingSignature(hash_hmac('sha256', $payload, $this->secret)); // Missing 'sha256=' prefix
    ...
    it('throws exception for wrong prefix', function () {
        $payload = '{"action": "opened"}';
        $signature = incomingSignature('sha1='.hash_hmac('sha256', $payload, $this->secret));
    ...
    it('throws exception for invalid signature', function () {
        $payload = '{"action": "opened"}';
        $signature = incomingSignature('sha256=invalid_hash_value');
    ...
    it('throws exception for signature with wrong secret', function () {
        $payload = '{"action": "opened"}';
        $signature = incomingSignature('sha256='.hash_hmac('sha256', $payload, 'wrong_secret'));
